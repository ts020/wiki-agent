...
use md_wiki::agentic_search_gate::{GateMode, GateOptions, run_gate};
...
fn parse_args() -> Result<GateOptions> {
    ...
    let mut work_dir = PathBuf::from("target/agentic-search-gate");
...
fn print_help() {
    println!(
        "agentic_search_gate\n\n\
         Usage:\n\
           cargo run --example agentic_search_gate -- --mode normal --work-dir DIR --report FILE [--min-score N]\n\n\
         Options:\n\
           --mode normal|heavy          Gate mode. Default: normal\n\
           --work-dir DIR               Temporary fixture/output dir. Default: target/agentic-search-gate\n\
