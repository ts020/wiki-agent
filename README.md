...
crates.io の package 名は `md-wiki-cli` ですが、binary name is still `md-wiki` です。GitHub Releases でバイナリを配布する場合は、対象 OS のアーカイブを展開して `md-wiki` を `PATH` の通った場所に置けば利用できます。
