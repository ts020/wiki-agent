...
`install.sh` は GitHub Release の `md-wiki-<target>.tar.gz` と `checksums.txt` を取得し、既定では `~/.local/bin` に配置します。場所を変える場合は `curl -fsSL https://raw.githubusercontent.com/ts020/wiki-agent/main/install.sh | MD_WIKI_INSTALL_DIR="$HOME/bin" sh`、特定 version を指定する場合は `curl -fsSL https://raw.githubusercontent.com/ts020/wiki-agent/main/install.sh | MD_WIKI_VERSION=v0.1.0 sh` を使います。

crates.io の package 名は `md-wiki-cli` ですが、binary name is still `md-wiki` です。GitHub Releases の手動インストールでは、対象 OS のアーカイブを展開して `md-wiki` を `PATH` の通った場所に置けば利用できます。
